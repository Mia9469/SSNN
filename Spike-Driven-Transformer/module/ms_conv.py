...
class MS_MLP_Conv(nn.Module):
    ...
    def forward(self, x, hook=None):
        ...
        x = self.fc1_lif(x)
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_fc1_lif"] = x
        ...
        x = self.fc2_lif(x)
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_fc2_lif"] = x
...
class MS_SSA_Conv(nn.Module):
    ...
    def forward(self, x, hook=None):
        ...
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_first_lif"] = x
        ...
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_q_lif"] = q_conv_out
        q = (
        ...
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_k_lif"] = k_conv_out
        k = (
        ...
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_v_lif"] = v_conv_out
        v = (
        ...
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_kv"] = kv
        x = q.mul(kv)
        if self.dvs:
            x = self.pool(x)
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_x_after_qkv"] = x
