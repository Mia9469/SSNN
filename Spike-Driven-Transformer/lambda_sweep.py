...
lambda_sweep.py
===============
P3.5: λ sensitivity sweep for CV regularisation on SDT / CIFAR-10.

Sweeps two families separately (to avoid blowing up to a 5×5 grid):
  Family A — WeightCV only    : λ_weight ∈ {1e-4, 5e-4, 1e-3, 5e-3, 1e-2}
  Family B — PopFR-CV only    : λ_pop    ∈ {1e-4, 2e-4, 5e-4, 1e-3, 2e-3}

For each λ we run a shorter 100-epoch training (enough for the ICE-vs-accuracy
trade-off to stabilise), and record the final {acc, mean_fr, weight_cv, cv_fr,
ce_loss, ice = acc / mean_fr}.

The output is a 2-panel Pareto plot:
  • ICE (y) vs Acc (x), one dot per λ
  • annotated with baseline (λ=0) for context

Runtime estimate: 10 runs × ~100 min = ~17 GPU-hours on a single 3090.
Drop `--epochs 60` if you want to finish overnight.

Usage
-----
    python lambda_sweep.py                          # full 100-ep sweep + plot
    python lambda_sweep.py --epochs 60              # faster sweep
    python lambda_sweep.py --family weight          # only family A
    python lambda_sweep.py --family pop             # only family B
    python lambda_sweep.py --plot-only              # re-render plot from JSON
"""
...
RESULTS_FILE = os.path.join(REPO, "lambda_sweep_results.json")
FIGURE_FILE  = os.path.join(REPO, "lambda_sweep.png")
...
# ── Sweep driver ──────────────────────────────────────────────────────────────
def run_sweep() -> dict:
    results: dict = {"family_weight": [], "family_pop": [], "epochs": N_EPOCHS}

    if CLI.family in ("weight", "both"):
        print("\n=== Family A: WeightCV sweep ===")
