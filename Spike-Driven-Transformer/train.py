...
# Import v2 criterion if available (for advanced CV loss)
try:
    import criterion_v2
    HAS_CV_V2 = True
except ImportError:
...
try:
    if torch.cuda.is_available() and getattr(torch.cuda.amp, "autocast", None) is not None:
        has_native_amp = True
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Enable firing rate CV maximization regularization loss (default: False)",
)
parser.add_argument(
    "--lambda-cv",
    type=float,
    default=0.01,
    metavar="LAMBDA",
    help="Strength of CV regularization term (default: 0.01)",
)
parser.add_argument(
    "--cv-weight",
    type=float,
    default=1.0,
    metavar="WEIGHT",
    help="Relative weight of CV loss term vs classification loss (default: 1.0)",
)
parser.add_argument(
    "--use-weight-cv",
    action="store_true",
    default=False,
    help="Enable weight parameter CV maximization (Sparse Coding inspired, default: False)",
)
parser.add_argument(
    "--lambda-weight-cv",
    type=float,
    default=0.001,
    metavar="LAMBDA",
    help="Strength of weight CV regularization term (default: 0.001)",
)
parser.add_argument(
    "--use-act-cv",
    action="store_true",
    default=False,
    help="Enable hidden layer activation CV maximization (default: False)",
)
parser.add_argument(
    "--lambda-act-cv",
    type=float,
    default=0.005,
    metavar="LAMBDA",
    help="Strength of activation CV regularization term (default: 0.005)",
)
parser.add_argument(
    "--spike-mode",
    default="lif",
...
parser.add_argument(
    ...
    default=0,
    metavar="N",
    help="limit train batches per epoch for quick debugging/ablation (0 = no limit)",
)
parser.add_argument(
    "--max-val-batches",
    type=int,
    default=0,
    metavar="N",
    help="limit validation batches for quick debugging/ablation (0 = no limit)",
)
parser.add_argument(
    "--recovery-interval",
    type=int,
...
def main():
    ...
    _logger.info(f"Using device: {_device}")
    ...
    if args.local_rank == 0:
        ...
        if torchinfo is not None:
            ...
            _logger.warning(
                "torchinfo is not installed; skipping model summary. "
                "Install it with `pip install torchinfo` to restore summary output."
            )
    ...
    if use_amp == "apex":
        ...
        if torch.cuda.is_available():
            ...
            amp_autocast = suppress  # AMP not well supported on MPS/CPU, disable
            _logger.info("AMP disabled on non-CUDA device.")
    ...
    train_loss_fn = train_loss_fn.to(_device)
    validate_loss_fn = nn.CrossEntropyLoss().to(_device)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if not args.prefetcher or args.dataset in dvs_utils.DVS_DATASET:
            if args.amp and torch.cuda.is_available() and not isinstance(input, torch.cuda.HalfTensor):
                input = input.half()
            input, target = input.to(_dev), target.to(_dev)
        ...
        # Collect LIF spike tensors via hook when any spike-based CV loss is enabled.
        use_cv_v2 = HAS_CV_V2 and (args.use_weight_cv or args.use_act_cv or args.use_cv_loss)
        need_hook = args.TET and use_cv_v2 and (args.use_cv_loss or args.use_act_cv)
        hook_dict = {} if need_hook else None

        with amp_autocast():
            output, hook_dict = model(input, hook=hook_dict)
            if args.TET:
                if use_cv_v2:
                    # Use criterion_v2 for multi-level CV regularization.
                    # hook_dict carries actual LIF spike tensors so CV is computed
                    # on real spikes, not on class logits.
                    loss = criterion_v2.combined_loss(
                        output, target, loss_fn,
                        means=args.TET_means,
                        lamb=args.TET_lamb,
                        use_cv_loss=args.use_cv_loss,
                        use_weight_cv=args.use_weight_cv,
                        use_act_cv=args.use_act_cv,
                        lambda_cv=args.lambda_cv,
                        lambda_weight_cv=args.lambda_weight_cv,
                        lambda_act_cv=args.lambda_act_cv,
                        cv_weight=args.cv_weight,
                        model=model,
                        hook=hook_dict,
                    )
                elif args.use_cv_loss:
                    # Use criterion.combined_loss for original CV regularization
                    loss = criterion.combined_loss(
                        output, target, loss_fn, 
                        means=args.TET_means, 
                        lamb=args.TET_lamb,
                        use_cv_loss=True,
                    ...
                    # Use original TET loss without CV
        ...
        if args.max_train_batches > 0 and (batch_idx + 1) >= args.max_train_batches:
            if args.local_rank == 0:
                _logger.info(
                    f"Early stop train epoch at batch {batch_idx + 1} due to --max-train-batches={args.max_train_batches}"
                )
            break
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.max_val_batches > 0 and (batch_idx + 1) >= args.max_val_batches:
                if args.local_rank == 0:
                    _logger.info(
                        f"Early stop validation at batch {batch_idx + 1} due to --max-val-batches={args.max_val_batches}"
                    )
                break
